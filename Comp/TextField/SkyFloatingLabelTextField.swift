 ...
 */
@IBDesignable
open class SkyFloatingLabelTextField: JMMaskTextField { // swiftlint:disable:this type_body_length
    /**
    ...
    @objc dynamic open var titleFadeInDuration: TimeInterval = 0.3
    ...
    public var withFixedTitle: Bool = false
    ...
    fileprivate func updatePlaceholder() {
        guard let placeholder = placeholder, let font = placeholderFont ?? font else { return }
    ...
    /// Discard error color of text
    open var discardTextErrorColor: Bool = false
    // MARK: Properties
    ...
    open var customTextColorControl: Bool {
        return false
    }
    ...
    fileprivate func updateLineView() {
        guard let lineView = lineView else { return }
    ...
    private func updateTextColor() {
        if customTextColorControl {
            return
        }
        if !isEnabled {
            super.textColor = disabledColor
        } else if hasErrorMessage {
            if !discardTextErrorColor {
    ...
    func updateTitleLabel(_ animated: Bool = false) {
        guard let titleLabel = titleLabel else { return }
    ...
    fileprivate func updateTitleVisibility(_ animated: Bool = false, completion: ((_ completed: Bool) -> Void)? = nil) {
        let alpha: CGFloat = isTitleVisible() ? 1.0 : (withFixedTitle ? 1.0 : 0.0)
        let frame: CGRect = titleLabelRectForBounds(bounds, editing: isTitleVisible())
        if self.isAnimationDisabled {
